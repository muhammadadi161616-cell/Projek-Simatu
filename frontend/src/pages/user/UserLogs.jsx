...
const UserLogs = () => {
    ...
    const fetchLogs = async () => {
        ...
        try {
            ...
            setError(err.message || 'Gagal memuat riwayat aktivitas Anda.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">Riwayat Aktivitas Saya</h2>
                    <p className="text-sm text-slate-400 font-medium">Daftar tindakan autentikasi dan perubahan tugas yang Anda lakukan.</p>
                </div>
                <button
                    onClick={fetchLogs}
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-16 bg-white border border-slate-200 rounded-2xl shadow-sm">
                    ...
                    <span className="text-xs font-semibold text-slate-500">Memuat riwayat aktivitas...</span>
                </div>
            ) : logs.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 text-center bg-white border border-slate-200 rounded-2xl shadow-sm">
                    ...
                    <p className="text-xs text-slate-400 mt-1 max-w-xs">
                        Seluruh aktivitas Anda akan dicatat di sini secara otomatis.
                    </p>
                </div>
            ) : (

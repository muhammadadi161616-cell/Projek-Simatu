...
import { History, RefreshCw, Calendar, User, Info } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">Riwayat Aktivitas</h2>
                    <p className="text-sm text-slate-400 font-medium">Log aktivitas sistem yang dilakukan oleh pengguna.</p>
                </div>
                <button
                    onClick={fetchLogs}
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-16 bg-white border border-slate-200 rounded-2xl shadow-sm">
                    ...
                    <span className="text-xs font-semibold text-slate-500">Memuat log aktivitas...</span>
                </div>
            ) : logs.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 text-center bg-white border border-slate-200 rounded-2xl shadow-sm">
                    ...
                    <h4 className="text-sm font-bold text-slate-700">Tidak ada riwayat aktivitas</h4>
                    <p className="text-xs text-slate-400 mt-1 max-w-xs">
                        Seluruh aktivitas autentikasi dan operasi tugas Anda akan dicatat di sini.
                    </p>
                </div>
            ) : (
                            ...
                            <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                                {logs.map((log) => (
                                    <tr key={log.id} className="hover:bg-slate-50/20 transition-colors">
                                        ...
                                        {/* Action Type */}
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex px-2.5 py-0.5 text-xs font-extrabold border rounded-lg uppercase tracking-wide font-mono ${getActionBadge(log.action)}`}>
                                                {log.action}
                                            </span>
                                        </td>
                                        {/* Log details */}

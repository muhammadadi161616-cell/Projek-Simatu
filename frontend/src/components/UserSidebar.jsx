...
const UserSidebar = ({ isOpen, toggleSidebar }) => {
    ...
    const menuItems = [
        { name: 'Dashboard Saya', path: '/user/dashboard', icon: LayoutDashboard },
        { name: 'Tugas Saya', path: '/user/tasks', icon: CheckSquare },
    ];

    const activeClassName = "flex items-center px-4 py-3 text-indigo-600 bg-indigo-50 border-r-4 border-indigo-600 font-semibold transition-all duration-200";
    const inactiveClassName = "flex items-center px-4 py-3 text-slate-600 hover:text-indigo-600 hover:bg-slate-50 font-medium transition-all duration-200";
    ...
    return (
        <>
            ...
            <aside className={`fixed top-0 bottom-0 left-0 z-50 flex flex-col w-64 bg-white border-r border-slate-200 transition-transform duration-300 transform lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                {/* User info */}
                <div className="p-4 mx-4 my-4 rounded-xl bg-slate-50 border border-slate-150">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center justify-center w-10 h-10 bg-indigo-100 text-indigo-700 font-bold rounded-full uppercase">
                            {user?.name?.charAt(0) || 'U'}
                        </div>
                        <div className="overflow-hidden">
                            <h4 className="text-sm font-semibold text-slate-700 truncate">{user?.name}</h4>
                            <span className="inline-block px-2 py-0.5 mt-0.5 text-[9px] font-extrabold text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-full uppercase tracking-wider">
                                {user?.role}
                            </span>
                        </div>
                    </div>
